...
const CONFIG = {
  ...
  FONTS: {
    family: 'Arial',
    sizes: {
      title: 45, date: 16, sectionTitle: 38, contentTitle: 28, subhead: 18,
...
function createDiagramSlide(slide, data, layout, pageNum) {
  ...
  const laneGap = px(CONFIG.DIAGRAM.laneGap_px), lanePad = px(CONFIG.DIAGRAM.lanePad_px), laneTitleH = px(CONFIG.DIAGRAM.laneTitle_h_px);
  const cardGap = px(CONFIG.DIAGRAM.cardGap_px), cardMinH = px(CONFIG.DIAGRAM.cardMin_h_px), cardMaxH = px(CONFIG.DIAGRAM.cardMax_h_px);
  const arrowH = px(CONFIG.DIAGRAM.arrow_h_px), arrowGap = px(CONFIG.DIAGRAM.arrowGap_px);
...
function createTableSlide(slide, data, layout, pageNum) {
  ...
  try {
    ...
    } else { throw new Error('headers is empty'); }
  } catch (e) {
...
export const generateSlideDataFromText = async (text: string): Promise<string> => {
  try {
    const fullPrompt = `${GENERATION_PROMPT}

---
Based on all the rules, schemas, and the blueprint provided above, analyze the following user input text. Your final output must be the single, complete Google Apps Script file, created by replacing the sample \`slideData\` in the GOOGLE_TEMPLATE_BLUEPRINT with the data you generate from the user text. Do not output any other text, explanations, or markdown formatting.
...
`;
    ...
    if (resultText.startsWith('```javascript')) {
      resultText = resultText.substring(13);
    } else if (resultText.startsWith('```gas')) {
        resultText = resultText.substring(5);
    ...
    throw new Error("AIとの通信で問題が発生しました。APIキーを確認して再試行してください。");
  }
};
